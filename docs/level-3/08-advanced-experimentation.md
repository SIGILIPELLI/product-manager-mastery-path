...
Sequential testing and multi-armed bandits solve a real cost problem in classic fixed-horizon A/B testing: checking results early and stopping as soon as they look significant ("peeking") inflates the true false-positive rate far above the nominal 5%, because you're implicitly running many tests (one at each peek) and taking the first lucky one — sequential testing methods (like mSPRT) solve this by using error-spending functions that mathematically account for the repeated looks, letting you check results continuously without inflating false positives, at the cost of needing tighter statistical machinery than a simple fixed-sample z-test. Multi-armed bandits work by treating experimentation as an explore/exploit tradeoff rather than a pure hypothesis test: instead of splitting traffic 50/50 for a fixed duration and only then acting on the winner, a bandit algorithm (e.g., Thompson Sampling) continuously shifts more traffic toward the currently-better-performing variant while still allocating some traffic to keep learning, which mathematically minimizes "regret" (the cumulative cost of showing users the worse variant) — the tradeoff is that bandits are worse than fixed A/B tests at producing a clean, generalizable causal estimate of effect size, because the traffic split itself changes based on the data, contaminating a straightforward significance calculation. Interaction effects between simultaneous experiments are a real statistical risk, not a theoretical one: if two concurrent tests both modify the same funnel step, their combined effect on a shared metric may not be the sum of their individual effects (a form of statistical interaction), which is why experimentation platforms mutually exclude tests that touch overlapping user flows or use factorial designs that can actually measure interaction terms rather than assuming independence.
