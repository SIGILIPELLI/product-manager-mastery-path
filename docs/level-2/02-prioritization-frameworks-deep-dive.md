...
Weighted scoring frameworks (RICE, ICE, etc.) don't just rank items — they change *which* items win by altering the functional form of the tradeoff. RICE = (Reach × Impact × Confidence) / Effort is a multiplicative model, which means it mathematically punishes items that are weak on any single factor far more harshly than an additive model would — a huge-impact idea with near-zero reach still scores low, correctly reflecting that impact without reach is not organizational impact. This is also why RICE is *sensitive to Confidence as a discount factor*: multiplying by a 20%-80% confidence score explicitly shrinks the expected value of unvalidated bets, which mathematically encodes the same discounting a rational expected-value calculation would apply to a risky asset, and is the reason RICE tends to systematically favor validated, incremental work over speculative moonshots unless Impact is scored high enough to compensate — a known distortion worth watching for. The Kano model works by exploiting a real, measured psychological asymmetry: satisfaction from "delighters" is unbounded and grows with investment, while dissatisfaction from missing "must-haves" is a step function (any amount below threshold causes disproportionate anger) — this is loss aversion showing up in feature perception, and it's why teams that only track "average satisfaction" miss that under-investing in a must-have can tank retention even while overall sentiment scores look fine. Cost of Delay / WSJF works because it makes the *time value* of a decision explicit — economically, delaying a high cost-of-delay item is not neutral, it's actively destroying value every week (like unpaid interest accruing), while WSJF's job-size denominator ensures short jobs with high urgency get pulled forward even over larger, more "important-sounding" ones, correctly modeling that clearing a fast queue often beats context-switching onto one big item.
